...
class AuthUseCase:
    ...
    def registrar_paciente(self, db: Session, dados):
        ...
        novo_usuario = UserTable(email=dados.email, senha_hash=hash_senha, role="PACIENTE")
        
        db.add(novo_usuario)
        db.commit()
        db.refresh(novo_usuario)

        novo_paciente = PatientTable(user_id=novo_usuario.id, nome=dados.nome, idade=dados.idade)
